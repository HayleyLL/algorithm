...
const {CircularList, iterator} = require('../lib/circularLinkedList')

describe('#makeIterator()', function () {
    it('should return a CircularList iterator', function () {
...
describe('#push()', function () {
    it('should append element at list tail', function () {
        ...
        assert.deepEqual(list.head.next.value, 1);
        assert.deepEqual(list.tail.value + list.head.value, 3);
    });
});
...
describe('pop()', function () {
    it('should remove and return the last value', function () {
            ...
            list.push(44);
            assert.deepEqual(list.tail.value, 44);
            assert.deepEqual(list.tail.next, list.head);
...
it('should implement concat', function () {
        let list1 = new CircularList();
        let list2 = new CircularList();
        list1.push(1, 2, 'hello');
        list2.push(4, 5, 6, 7);

        const list = list1.concat(list2);
        assert.deepEqual(list1.size, 7);
        assert.deepEqual(list2.size, 7);
        assert.deepEqual(list.get(3), 4);
        assert.deepEqual(list.tail.next, list.head);
        assert.deepEqual(list.tail.value, 7);
        assert.deepEqual(list.head.value, 1);
        assert.deepEqual(list.get(6), 7);
    }
);